...
- 💿 **Singles → Albums** : un morceau téléchargé comme single est automatiquement **déplacé dans son album** (dossier + métadonnées : album, numéro de piste, pochette) quand une playlist/l'album le contenant est téléchargé ensuite — **sans re-télécharger l'audio**. Ses **stats suivent le déplacement** : historique d'écoutes, likes et playlists sont migrés vers le nouveau chemin (identité = URL Spotify dans les tags).
- 🏷️ **Marqueur soneph** : chaque fichier téléchargé reçoit des tags `TXXX:SONEPH` + `TXXX:SONEPH_SOURCE` (la source du téléchargement), pour que l'app sache d'où vient chaque morceau même après déplacement.
- 🔍 **Plus de détails (clic droit)** : panneau avec les métadonnées ID3 complètes — qualité, source des paroles, artistes, producteurs, musiciens, éditeur, lien Spotify…
- 🎤 **Paroles intelligentes** : les paroles synchronisées (avec horodatage) sont considérées comme le meilleur dispo et ne sont **pas re-téléchargées** ; les fichiers en texte brut sont automatiquement upgradés vers la version synced. La source est inscrite dans les tags (`TXXX:LYRICS_SOURCE` = `lrclib`, `musixmatch`…) et la qualité réelle dans `TXXX:SONEPH_QUALITY` (ex. `320kbps`).
...
**Coller un lien de playlist Spotify télécharge les sons ET crée la playlist en même temps** : les morceaux déjà sur disque sont ajoutés immédiatement (identifiés par leur URL Spotify dans les tags, pas par le nom), les manquants sont téléchargés puis ajoutés automatiquement à la fin. Aucun re-téléchargement inutile, aucun toggle à activer. Les liens d'album téléchargent simplement l'album (l'app le regroupe déjà par album).
